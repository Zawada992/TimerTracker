const apikey = '9327a835-7300-41f2-b68f-b823fbe57828';
...
function renderTask(taskId, title, description, status) {
    ...
    if (status === 'open') {
        ...
        finishButton.addEventListener('click', ev => {
            ...
            section.querySelectorAll(".js-task-open-only").forEach(e =>{
               e.remove();
            })
    ...
    if (status === 'open') {
        ...
        addOperationDiv.className = 'cart-body js-task-open-only';
        ...
        descriptionInput.setAttribute('id', 'id-descIn');
...
function renderOperation(ul, status, operationId, operationDescription, timeSpent) {
    ...
    time.innerText = formatTime(timeSpent) + 'm';
    ...
    if (status == "open") {
        ...
        controlDiv.className = "js-open";
        li.appendChild(controlDiv);

        const add15minB = document.createElement('button');
        add15minB.className = 'btn btn-outline-success btn-sm mr-2 js-task-open-only';
        add15minB.innerText = '+15min';
        controlDiv.appendChild(add15minB);
        add15minB.addEventListener('click', function() {
        ...
        const add1hB = document.createElement('button');
        add1hB.className = 'btn btn-outline-success btn-sm mr-2 js-task-open-only';
        add1hB.innerText = '+1h';
        controlDiv.appendChild(add1hB);
        add1hB.addEventListener('click', function() {
        ...
        deleteB.className = 'btn btn-outline-danger btn-sm js-task-open-only';
        deleteB.innerText = 'Delete';
        controlDiv.appendChild(deleteB);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelector('.js-task-adding-form').addEventListener("submit", function (ev){
        ev.preventDefault();
        ...
        apiCreateTask(ev.target.elements.title.value, ev.target.elements.description.value)
            .then(
                function (res) { renderTask(res.data.id, res.data.title, res.data.description, res.data.status);
                title.value = "";
                desc.value = "";
